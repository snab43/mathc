...
/*
    Can't be used outside of a dialog object. Used to
    take logic out of the dialog object step event.
*/
...
if (string_length(new_string) > string_length(text_visible)) {
    ...
    // Check for a pause
    if (is_in_array(last_char, array(".", "!", "?", "~", "-")) and text_count < string_length(text[text_page])) {
        alarm[0] = pause_time;
    }
    ...
    // Play blip sound
    audio_play_sound_effect(sound, false, 50);
}
