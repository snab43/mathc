...
stats_object = id;
if (!is_enemy) {
    // Set up the stats for the player
    stats_object = o_player_stats;
    
    var weapon_stats = o_data.equipment[? o_player_stats.weapon];
    var armor_stats = o_data.equipment[? o_player_stats.armor];
    atk_mod = weapon_stats[? "attack"]+armor_stats[? "attack"];
    def_mod = weapon_stats[? "defense"]+armor_stats[? "defense"];
    spd_mod = weapon_stats[? "speed"]+armor_stats[? "speed"];
    crt_mod = weapon_stats[? "critical"]+armor_stats[? "critical"];
} else {
